...
class ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, activation='relu', norm=None):
        ...
        nn.init.xavier_uniform(self.conv3d.weight)
...
class ImageTransformer(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.last_fc1 = nn.Linear(3*256, 3*16, bias = True)
        self.last_fc2 = nn.Linear(3*16, 3, bias = True)
        nn.init.xavier_uniform(self.last_fc1.weight)
        nn.init.xavier_uniform(self.last_fc2.weight)
        self.last_conv = ConvLayer(in_channels=3,out_channels=1,kernel_size=5,stride=1,padding=2)
    ...
    def forward(self, X, sampling=False):
        ...
        X = torch.reshape(X,(64*64,3*256))
        X = self.last_fc1(X)
        X = self.last_fc2(X)
        X = torch.reshape(X,(1,3,64,64,1))
        X = self.last_conv(X)
